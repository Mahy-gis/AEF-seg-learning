...
from seg import train_unet_from_mts12_raw as base_train
...
def _pick_or_default(raw: str, default_value: object, cast: Callable[[str], object]) -> List[object]:
    values = _parse_list(raw, cast)
    return values if values else [default_value]
...
def _parse_search_args() -> tuple[argparse.Namespace, argparse.Namespace]:
    ...
    search_parser.add_argument("--search_time_steps", type=str, default="")
    search_parser.add_argument("--search_tag", type=str, default="search_raw")
    ...
    search_parser.add_argument(
        ...
        type=int,
        default=0,
        help="If >0, stop after max_trials combinations.",
    )
...
def _score_is_better(objective: str, score: float, best_score: float) -> bool:
    if objective == "best_val_loss":
...
def main() -> None:
    ...
    lr_list = _pick_or_default(search_args.search_lrs, float(base_args.lr), float)
    dice_list = _pick_or_default(search_args.search_dice_weights, float(base_args.dice_weight), float)
    crop_list = _pick_or_default(search_args.search_crop_sizes, int(base_args.train_crop_size), int)
    model_variant_list = _pick_or_default(search_args.search_model_variants, str(base_args.model_variant), str)
    unet_depth_list = _pick_or_default(search_args.search_unet_depths, int(base_args.unet_depth), int)
    base_channels_list = _pick_or_default(search_args.search_base_channels, int(base_args.base_channels), int)
    use_fg_sampler_list = _pick_or_default(search_args.search_use_fg_sampler, int(base_args.use_fg_sampler), int)
    ignore_bg_dice_list = _pick_or_default(
        search_args.search_ignore_bg_dice,
        int(base_args.ignore_background_in_dice),
        int,
    )
    ignore_bg_metrics_list = _pick_or_default(
        search_args.search_ignore_bg_metrics,
        int(base_args.ignore_background_in_metrics),
        int,
    )
    focal_focus_classes_list = _pick_or_default(
        search_args.search_focal_focus_classes,
        str(base_args.focal_focus_classes),
        str,
    )
    time_steps_list = _pick_or_default(search_args.search_time_steps, int(base_args.time_steps), int)

    combos: Sequence[tuple[object, ...]] = list(
        itertools.product(
            lr_list,
            dice_list,
            crop_list,
    ...
    print("==== Raw UNet Hyper-Parameter Search ====")
    ...
    results = []
    best_cfg = None
    best_score = float("inf") if search_args.objective == "best_val_loss" else float("-inf")
    ...
    for idx, combo in enumerate(combos, start=1):
        ...
        trial_args.time_steps = int(time_steps)
        ...
        subdir_name = (
            ...
            f"fgs{int(use_fg_sampler)}_igd{int(ignore_bg_dice)}_igm{int(ignore_bg_metrics)}_"
            f"ts{int(time_steps)}"
        )
        trial_args.output_dir = str(root_out / subdir_name)

        print("\n---- Trial {}/{} ----".format(idx, len(combos)))
        print(
            "cfg:",
            {
                "lr": lr,
        ...
        if _score_is_better(search_args.objective, score, best_score):
            best_score = score
